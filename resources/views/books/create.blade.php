    ...
    <div class="container py-4">
        ...
        <div class="form-shell">
            <div class="form-header">
                <h1 class="h3 mb-1"><i class="bi bi-plus-circle"></i> Create Book</h1>
                <p class="mb-0">Add a new book record to the store.</p>
            </div>
            ...
            <div class="form-body">
                ...
                <form action="{{ route('books.ui.store') }}" method="POST" class="row g-4">
                    @csrf

                    <div class="col-lg-8">
                        <label class="form-label">Title</label>
                        <input type="text" name="title" value="{{ old('title') }}" class="form-control @error('title') is-invalid @enderror" placeholder="Book title">
                        @error('title')
                    ...
                    <div class="col-lg-4">
                        ...
                        <input type="number" name="price" value="{{ old('price') }}" step="0.01" min="0" class="form-control @error('price') is-invalid @enderror" placeholder="0.00">
                        @error('price')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" rows="5" class="form-control @error('description') is-invalid @enderror" placeholder="Book description">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
